...
- Если оговорено использование [retina.js](http://imulus.github.io/retinajs/) необходимо создать изображения в двойном размене с префиксом `@2x` перед расширением и находиться рядом с оригиналом. Пример: `my_image@2x.png`
...
- Динамические элементы должны работать, под все подразумевающиеся callback'и должны быть выделены готовые функции. Например: есть блок, в котором по клику на "плюс" должен появиться элемент - эта функция должна быть описана. Если при создании блока подразумевается ajax запрос должен стоять коментарий, где его инициализировать. Если по клику на этот элемент должно появиться модальное окно - эти функции должны быть описаны. Если подразумевается, что перед открытием необходимо получить данные через ajax запрос должен стоять коментарий, где его инициализировать. Если по любому действию над модальным окном должен происходить ajax запрос должен стоять коментарий, где его инициализировать
...
- Скрипты запрещено мизимизировать, обфусцировать и производить любые действия которые затрудняют дальнешее чтение кода.
...
Можно использовать любой из предложенных препроцессоров. В окончательной версии файлы должны быть скомпилированы в статичный CSS.
